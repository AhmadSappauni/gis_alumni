...
@push('styles')
    ...
    <style>
        ...
        .btn-navigation {
            ...
            justify-content: space-between !important; /* Ini kuncinya */
        ...
        #nextBtn {
            margin-left: auto; /* Memaksa tombol Lanjut ke kanan jika tombol Sebelumnya sembunyi */
...
<div class="glass-panel" style="padding: 40px; max-width: 1000px; margin: 0 auto;">
    ...
    <form action="{{ route('admin.alumni.update', $alumni->nim) }}" method="POST" enctype="multipart/form-data" id="wizardForm">
        @csrf
        @method('PUT')
        ...
        <div class="form-step active" id="step1">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div>
                    ...
                    <input type="text" name="nim" id="nim" class="custom-input-admin" value="{{ old('nim', $alumni->nim) }}" placeholder="211013...">
                    <small id="nim-status"></small>
                </div>
                <div>
                    <label class="label-admin">Nama Lengkap</label>
                    <input type="text" name="nama_lengkap" class="custom-input-admin" value="{{ old('nama_lengkap', $alumni->nama_lengkap) }}" required>
                </div>
                <div>
                    <label class="label-admin">Angkatan</label>
                    <input type="number" name="angkatan" class="custom-input-admin" value="{{ old('angkatan', $alumni->angkatan) }}">
                </div>
                <div>
                    <label class="label-admin">Tahun Lulus</label>
                    <input type="number" name="tahun_lulus" class="custom-input-admin" value="{{ old('tahun_lulus', $alumni->tahun_lulus) }}">
                </div>
            </div>
            ...
            <div style="margin-top: 20px; background: rgba(0,74,135,0.03); padding: 20px; border-radius: 15px;">
                ...
                <div style="display: flex; align-items: center; gap: 20px;">
                    <div style="position: relative;">
                        ...
                        <div style="position: absolute; bottom: -10px; left: 50%; transform: translateX(-50%); background: #004a87; color: white; padding: 2px 10px; border-radius: 10px; font-size: 10px; white-space: nowrap;">
                            Foto Saat Ini
                        </div>
                    </div>
        ...
        <div class="form-step" id="step2">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div>
                    ...
                    <input type="text" name="jabatan" class="custom-input-admin" value="{{ old('jabatan', $alumni->pekerjaan->jabatan ?? '') }}" placeholder="Software Engineer">
                </div>
                <div>
                    ...
                    <select name="bidang" class="custom-input-admin">
                        @foreach(['IT & Software', 'Pendidikan / Guru', 'Pemerintahan', 'Wiraswasta'] as $bidang)
                            <option value="{{ $bidang }}" {{ (old('bidang', $alumni->pekerjaan->bidang_pekerjaan ?? '') == $bidang) ? 'selected' : '' }}>{{ $bidang }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="linearitas" class="custom-input-admin">
                        <option value="Linier" {{ (old('linearitas', $alumni->pekerjaan->linearitas ?? '') == 'Linier') ? 'selected' : '' }}>Linier</option>
                        <option value="Tidak Linier" {{ (old('linearitas', $alumni->pekerjaan->linearitas ?? '') == 'Tidak Linier') ? 'selected' : '' }}>Tidak Linier</option>
                    </select>
                </div>
                <div>
                    <label class="label-admin">Nama Perusahaan</label>
                    <input type="text" name="nama_perusahaan" class="custom-input-admin" value="{{ old('nama_perusahaan', $alumni->pekerjaan->nama_perusahaan ?? '') }}">
                </div>
                <div>
                    <label class="label-admin">Estimasi Gaji (Opsional)</label>
                    <input type="text" name="gaji" class="custom-input-admin" value="{{ old('gaji', $alumni->pekerjaan->gaji ?? '') }}" placeholder="Rp 5.000.000">
                </div>
            </div>
            <div style="margin-top: 20px;">
                <label class="label-admin">Link LinkedIn</label>
                <input type="url" name="linkedin" class="custom-input-admin" value="{{ old('linkedin', $alumni->pekerjaan->link_linkedin ?? '') }}" placeholder="https://linkedin.com/in/username">
            </div>
        </div>
    ...
    <div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 25px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 15px;">
            <div>
                ...
                <input type="text" name="kota" id="kota" class="custom-input-admin" value="{{ $alumni->pekerjaan->kota ?? '' }}">
            </div>
            <div>
            ...
            setTimeout(() => {
                if (typeof map !== 'undefined') {
                    ...
                    L.marker([oldLat, oldLng]).addTo(map)
                     .bindPopup("Lokasi saat ini: {{ $alumni->pekerjaan->nama_perusahaan ?? 'Alumni' }}")
                     .openPopup();
                }
            }, 800);
        });
